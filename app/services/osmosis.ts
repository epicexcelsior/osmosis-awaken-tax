 ...
 */
export async function testLCDEndpoint(
  address: string = "osmo1g8gv8ayl55698fscstajt9uqw9r6w8a34aly9v",
): Promise<any> {
...
export async function fetchAllTransactions(
  ...
  transactions: OsmosisTransaction[];
  metadata?: any;
  verification?: any;
}> {
  console.log(
    `[fetchAllTransactions] Fetching via Cloudflare Function for: ${address}`,
  );
  ...
  try {
    ...
    const data = await response.json();
    console.log(
      `[fetchAllTransactions] Fetched ${data.transactions?.length} transactions`,
    );
